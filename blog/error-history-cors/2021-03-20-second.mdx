...
로컬 환경에서의 CORS 에러 해결법은 이전 포스트 [CORS 에러와 이별하기(1)](https://sum-log.vercel.app/blog/2021/03/20/error-history-cors/first)를 참고해 주세요.
...
**Faa**의 대표적인 서비스에는 **AWS Lambda, Google Cloud Functions**가 있다.
...
결론부터 말하자면 FaaS 플랫폼의 **Serverless Functions를 사용해 서버에서 외부 API로 요청**을 보냄으로써 SOP, CORS 정책에 영향을 받지 않고 리소스를 얻을 수 있다. **SOP, CORS 정책을 지킨 응답인지 판별하는 주체는 서버가 아닌 브라우저**다. 서버 측에서는 우리가 요청한 데이터를 제공하지만, 출처가 동일하지 않거나 올바른 응답 헤더를 반환하지 않을 경우 브라우저에서 이를 차단하여 CORS 에러가 발생하는 것이다. **즉, 브라우저를 거치지 않고 서버에서 외부 API로 데이터를 요청할 수 있다면 SOP와 CORS 정책과 상관 없이 리소스를 받을 수 있다.**
